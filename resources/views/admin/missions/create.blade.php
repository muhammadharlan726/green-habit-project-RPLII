<x-admin-layout>
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">➕ Create New Mission</h1>
        <p class="text-gray-600 text-xs sm:text-sm mt-1">Tambahkan misi baru ke sistem</p>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <form action="{{ route('admin.missions.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required class="w-full px-3 sm:px-4 py-2 rounded-lg border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 transition text-sm @error('title') border-red-500 @enderror">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6" required placeholder="Jelaskan misi secara detail..." class="w-full px-3 sm:px-4 py-3 rounded-lg border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 transition text-sm font-sans resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <input type="number" id="points_reward" name="points_reward" value="{{ old('points_reward') }}" min="1" required class="w-full px-3 sm:px-4 py-2 rounded-lg border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 transition text-sm @error('points_reward') border-red-500 @enderror">
                    @error('points_reward')
                        <p class="text-red-500 text-xs sm:text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 hover:border-green-400 has-[:checked]:border-green-500 has-[:checked]:bg-green-50 has-[:checked]:shadow-md">
                            <input type="radio" name="frequency" value="daily" {{ old('frequency', 'daily') === 'daily' ? 'checked' : '' }} required class="w-5 h-5 text-green-600 focus:ring-2 focus:ring-green-500">
                            <div>
                        ...
                        <label class="relative flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 hover:border-blue-400 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 has-[:checked]:shadow-md">
                            <input type="radio" name="frequency" value="weekly" {{ old('frequency') === 'weekly' ? 'checked' : '' }} required class="w-5 h-5 text-blue-600 focus:ring-2 focus:ring-blue-500">
                            <div>
                ...
                <div class="flex items-start gap-3 p-3 sm:p-4 rounded-lg bg-gray-50 border">
                    <input type="checkbox" id="requires_evidence" name="requires_evidence" value="1" @if(old('requires_evidence')) checked @endif class="mt-1 w-4 h-4 sm:w-5 sm:h-5 rounded text-green-600 focus:ring-2 focus:ring-green-200">
                    <div>
